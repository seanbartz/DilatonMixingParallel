...
def sigma_values():
    print("The Quark Mass, Chemical Potential, Max Sigma, Lambda, and Delta Sigma will be a one time input. To change, restart the code")
    l = float(input("Quark Mass: "))
    m = float(input("Chemical Potential: "))
    y = float(input("Max Sigma: "))
    lam = float(input("Lambda Value: ")) #parameter for mixing between dilaton and chiral field
    global deltasig
    deltasig = float(input("Delta Sigma: "))
    while True:
            print("----------------------------------------------------------------------------------")
            tmin= float(input("Minimium Temperature(Press enter to quit): "))
            tmax= float(input("Maximum Temperature: "))
            numtemp= int(input("Temperature Incriments: "))
            ml=l*np.ones(numtemp)
            mu=m*np.ones(numtemp)
            lambda1=lam*np.ones(numtemp)
            temps=np.linspace(tmin,tmax,numtemp)

            minsigma=0*np.ones(numtemp)
            maxsigma=y*np.ones(numtemp)
