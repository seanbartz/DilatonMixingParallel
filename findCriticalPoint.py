...
# -*- coding: utf-8 -*-
"""
Created on Thursday January 5 2023
Split from criticalZoom.py

This version iterates on the value of mu to find the critical point for a given lambda1 and a0
...
def order_checker(tmin,tmax,numtemp,minsigma,maxsigma, ml, mu, lambda1,a0):
    ...
    if max(truesigma[:,1])==0:
        ...
        numtemp=len(temps)
        #find the temp value where the gradient of truesigma[:,0] is most negative, and the value of truesigma[:,0] is not zero
        #this is the pseudo-critical temperature
        transitionIndex=np.argmin(np.gradient(truesigma[:,0]))
        buffer=2
        ...
        criticalTemp=temps[transitionIndex]
        ...
        print("Critical temperature for mu= ", mu[0], " is ", temps[np.argmax(truesigma[:,1])] )
        criticalTemp= temps[np.argmax(truesigma[:,1])]
        order=1
    return tmin,tmax,minsigma,maxsigma,order,temps,truesigma,criticalTemp

@timebudget
def crossover_finder(ml,lambda1,mu,a0,tmin,tmax,numtemp):
    minsigma=0
    maxsigma=200
    order=2
    iterationNumber=0
    ...
    while order==2 and iterationNumber<10 and tmin<tmax and maxsigma-minsigma>2:
        tmin,tmax,minsigma,maxsigma,order,temps,truesigma,criticalTemp=order_checker(tmin,tmax,numtemp,minsigma,maxsigma,ml,mu,lambda1,a0)
        iterationNumber=iterationNumber+1
        print("Iteration number ", iterationNumber)

        if tmax<tmin:
            print("TEMPERATURE BOUNDS REVERSED!!!")
    return order,temps,truesigma,criticalTemp
...
if __name__ == '__main__':
    ...
    #load the previously calculated critical points
    criticalPoints=np.load("criticalPoints.npy")
    ...
    lambda1= 6.2 #parameter for mixing between dilaton and chiral field
    ...
    order=1
    deltaMu=5
    # decrease mu by deltaMu until the transition is second order
    while order==1:
        if muchecks>0:
            oldtemps=temps
            oldtruesigma=truesigma
            oldcriticalTemp=criticalTemp
        order,temps,truesigma,criticalTemp=crossover_finder(ml,lambda1,mu,a0,tmin,tmax,numtemp)
        mu=mu-deltaMu
        muchecks+=1
    print ("Critical point is at mu=", mu+2*deltaMu, "MeV, T=", oldcriticalTemp  )

    # append the critical point to the list of critical points
    criticalPoints=np.vstack((criticalPoints,[ml,lambda1,mu+2*deltaMu,mu+deltaMu,oldcriticalTemp,criticalTemp]))
    #save the critical points
    np.save("criticalPoints.npy",criticalPoints)
    
    # plot the crossover plot and the prior iteration's first order transition
    mu=mu+deltaMu
    if muchecks>0:
        #plot the sigma values for the previous iteration
        plt.scatter(oldtemps,oldtruesigma[:,0])
        plt.scatter(oldtemps,oldtruesigma[:,1])
        plt.scatter(oldtemps,oldtruesigma[:,2])
        plt.ylim([min(oldtruesigma[:,0])-2,max(oldtruesigma[:,0])+2])
        plt.xlim(oldtemps[0],oldtemps[len(oldtemps)-1])
        plt.xlabel('Temperature (MeV)')
        plt.ylabel(r'$\sigma^{1/3}$ (MeV)')
        plt.title(r'$m_q=%i$ MeV, $\mu=%i$ MeV, $\lambda_1=$ %f' %(ml,mu+5,lambda1))
        plt.show()

    #plot the sigma values for the last iteration
    plt.scatter(temps,truesigma[:,0])
    plt.scatter(temps,truesigma[:,1])
    plt.scatter(temps,truesigma[:,2])
    plt.ylim([min(truesigma[:,0])-2,max(truesigma[:,0])+2])
    plt.xlim(temps[0],temps[len(temps)-1])
    plt.xlabel('Temperature (MeV)')
    plt.ylabel(r'$\sigma^{1/3}$ (MeV)')
